...
//    If ThemisActionBuilder has invalid params for Action -> throws InvalidThemisActionException with message what was wrong;
//    If no template exist for given Action -> throws NoThemisActionTemplateFoundException;
    public static void validate(Action action, Params params) throws NoThemisActionTemplateFoundException, InvalidThemisActionException {
        ...
        if (template == null) {
            ...
            for (String param: params.keySet()) {
                if (!(Arrays.asList(requiredParams).contains(param) || Arrays.asList(optionalParams).contains(param) || param.equals("collectionName") || param.equals("actionName"))) {
