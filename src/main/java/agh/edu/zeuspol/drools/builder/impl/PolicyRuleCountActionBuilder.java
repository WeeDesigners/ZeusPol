...
public class PolicyRuleCountActionBuilder extends DrlActionBuilder {
    ...
    public String build() {
        ...
        drlStringBuilder.append("if ($stats.fireCount >= $stats.maxRetry) {\n");

        drlStringBuilder.append("""
                StalePolicyRuleNotification.sendNotification(%s, "%s", "%s", "%s");
                
                """.formatted(policyRule.id, policyRule.name, policyRule.getConditions().toString(), policyRule.action.toString()));
    ...
    private String valueComparisonString(int metricIndex, RelationType actionType) {
        return switch (actionType) {
            case GT -> "$stats.prevMetrics.get(%s).value > $m%s.value".formatted(metricIndex, metricIndex);
            case LT -> "$stats.prevMetrics.get(%s).value < $m%s.value".formatted(metricIndex, metricIndex);
