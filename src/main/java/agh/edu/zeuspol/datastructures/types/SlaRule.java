  ...
  public List<PolicyRule> toPolicyRules() {
    ...
    switch (this.relation) {
      case EQ -> {
        rules.add(
            new PolicyRule(
                attribute, subject, value, unit, RelationType.LT, Action.DEFAULT, new Params()));
        rules.add(
            new PolicyRule(
                attribute, subject, value, unit, RelationType.GT, Action.DEFAULT, new Params()));
      }
      case GT -> {
        rules.add(
            new PolicyRule(
        ...
        rules.add(
            new PolicyRule(
                attribute, subject, value, unit, RelationType.EQ, Action.DEFAULT, new Params()));
      }
      case LT -> {
        rules.add(
            new PolicyRule(
      ...
      case BT -> {
        ...
        rules.add(
            new PolicyRule(
                attribute, subject, n2, unit, RelationType.GT, Action.DEFAULT, new Params()));
        rules.add(
            new PolicyRule(
                attribute, subject, n1, unit, RelationType.LT, Action.DEFAULT, new Params()));
      }
    }
    return rules;
  }
